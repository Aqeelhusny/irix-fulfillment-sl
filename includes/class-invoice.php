...
final class WCFSL_Invoice {
	...
	public function maybe_render(): void {
		if ( ! isset( $_GET['wcfsl_invoice'] ) ) return; // phpcs:ignore
		...
		if ( $is_admin && isset( $_GET['nonce'] ) ) { // phpcs:ignore
			if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['nonce'] ) ), 'wcfsl_invoice_' . $order_id ) ) {
				wp_die( esc_html__( 'Security check failed.', 'wc-fulfillment-sl' ) );
			}
		} elseif ( ! $is_admin ) {
			if ( ! $order_key ) wp_die( esc_html__( 'Invalid request.', 'wc-fulfillment-sl' ) );
		} else {
			wp_die( esc_html__( 'Unauthorized.', 'wc-fulfillment-sl' ) );
		}

		$order = wc_get_order( $order_id );
		if ( ! $order ) wp_die( esc_html__( 'Order not found.', 'wc-fulfillment-sl' ) );

		if ( ! $is_admin && ! hash_equals( $order->get_order_key(), $order_key ) ) {
			wp_die( esc_html__( 'Access denied.', 'wc-fulfillment-sl' ) );
		}
	...
	public static function invoice_url( int $order_id, bool $admin = true, string $order_key = '' ): string {
		if ( $admin ) {
			return add_query_arg( [
				...
				'order_id'      => $order_id,
				'admin'         => '1',
				'nonce'         => wp_create_nonce( 'wcfsl_invoice_' . $order_id ),
			], home_url( '/' ) );
		}
		return add_query_arg( [
			'wcfsl_invoice' => '1',
